...
function arrayLimit(key: string | null) {
    if (key && ROW_ARRAY_KEYS.has(key))
        return LIMITS.rows;
    if (key && POINT_ARRAY_KEYS.has(key))
        return LIMITS.points;
    if (key && PLOT_ARRAY_KEYS.has(key))
        return LIMITS.plots;
    if (key && SERIES_ARRAY_KEYS.has(key))
        return LIMITS.seriesValues;
    if (key && MATRIX_ARRAY_KEYS.has(key))
        return LIMITS.matrixAxis;
    return LIMITS.seriesValues;
}
function boundedValue(value: unknown, key: string | null, depth: number, context: SnapshotContext): unknown {
    ...
    if (Array.isArray(value)) {
        ...
        if (value.length > limit)
            context.truncated = true;
        return value.slice(0, limit).map((item) => (boundedValue(item, null, depth + 1, context)));
    }
