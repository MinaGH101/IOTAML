...
function nodePortType(nodes: Node[], nodeId: string, handle: string | null | undefined, side: 'inputs' | 'outputs') {
...
export function analyzeComponentBoundary(nodes: Node[], edges: Edge[], selectedIds: string[]): ComponentBoundary | null {
    ...
    const adjacency = new Map(selectedNodes.map((node) => [node.id, new Set<string>()]));
