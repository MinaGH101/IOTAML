...
export function normalizeInteractiveTableState(value: unknown): InteractiveTableState {
    ...
    const edits = isRecord(source.cell_edits)
        ? Object.fromEntries(Object.entries(source.cell_edits)
...
export function initializeTableSelection(state: InteractiveTableState, rowKeys: string[], columns: string[]): InteractiveTableState {
    const normalized = normalizeInteractiveTableState(state);
    if (normalized.selection_initialized)
        return normalized;
    return {
        ...normalized,
...
export function applyInteractiveTableState(sourceRows: Record<string, unknown>[], sourceColumns: string[], value: InteractiveTableState) {
    const state = normalizeInteractiveTableState(value);
    const columns = [...sourceColumns];
    const rows: Array<Record<string, unknown> & { __row_key: string }> = sourceRows.map((row, index) => ({
        __row_key: `source:${index}`,
        ...row,
    }));
    (state.added_columns || []).forEach((definition) => {
        if (!definition.name || columns.includes(definition.name))
            return;
    ...
    const resultRows = (initialized
        ...
        : rows).map((row) => Object.fromEntries(resultColumns.map((column) => [column, row[column]])));
    return { rows, columns, resultRows, resultColumns };
}
export function withCellEdit(state: InteractiveTableState, rowKey: string, column: string, value: unknown): InteractiveTableState {
    return {
        ...state,
        version: 1,
        cell_edits: {
