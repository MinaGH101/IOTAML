...
export function buildBoardOutputLookup(outputs: Output[]) {
    ...
    outputs.forEach((output) => {
        ...
        values.push(output);
        if (Array.isArray(output.plots))
            values.push(...(output.plots as Output[]));
        byNode.set(nodeId, values);
    });
    return byNode;
}
export function findCurrentBoardOutput(item: AnalysisBoardItem, lookup: Map<string, Output[]>) {
    ...
    const exact = searchable.find((output) => String(output.title || '') === item.outputTitle
...
export function resolveBoardItems(items: AnalysisBoardItem[], outputs: Output[], workflowDirty: boolean) {
