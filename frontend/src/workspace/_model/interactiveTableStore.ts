...
function sourceFromOutput(output: Output): InteractiveTableSource | null {
  if (String(output.kind || '') !== 'interactive_table') return null;
...
export class InteractiveTableRuntime {
  ...
  subscribe = (nodeId: string, listener: () => void) => {
    ...
    return () => {
      ...
      if (!current.size) this.listeners.delete(nodeId);
    };
  };
  ...
  hydrateNodes(nodes: Node[]) {
    nodes.forEach((node) => {
      if (String(node.data?.registryId || node.data?.catalogId || '') !== 'UT-008') return;
      const state = normalizeInteractiveTableState(
        (node.data?.params as Record<string, unknown> | undefined)?.table_state,
      );
      const incomingSignature = signature(state);
      if (incomingSignature === this.writtenSignatures.get(node.id)) return;
      const current = this.snapshot(node.id);
      if (signature(current.state) === incomingSignature) return;
  ...
  hydrateOutputs(outputs: Output[]) {
    outputs.forEach((output) => {
      ...
      if (signature(current.source) === signature(source)) return;
  ...
  setState(nodeId: string, value: InteractiveTableState) {
    ...
    const current = this.snapshot(nodeId);
    if (signature(current.state) === signature(state)) return;
