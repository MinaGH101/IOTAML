...
import { EMPTY_INTERACTIVE_TABLE_ENTRY, InteractiveTableRuntime, type InteractiveTableRuntimeEntry, } from './interactiveTableStore';
...
export function useInteractiveTableEntry(nodeId: string): InteractiveTableRuntimeEntry {
    ...
    return useSyncExternalStore((listener) => runtime?.subscribe(nodeId, listener) || (() => undefined), () => runtime?.snapshot(nodeId) || EMPTY_INTERACTIVE_TABLE_ENTRY, () => runtime?.snapshot(nodeId) || EMPTY_INTERACTIVE_TABLE_ENTRY);
}
