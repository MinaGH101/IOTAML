...
function updateNodeData(nodes: Node[], nodeId: string, patch: Record<string, unknown>) {
    return nodes.map((node) => (node.id === nodeId
        ? { ...node, data: { ...node.data, ...patch } }
        : node));
}
export function updateNodeParameters(nodes: Node[], nodeId: string, params: Record<string, unknown>) {
...
export function updateNodePinnedOutput(nodes: Node[], nodeId: string, pinned: PinnedNodeData) {
