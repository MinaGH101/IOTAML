...
import { runDuration } from '../../../../shared/lib/run';
export function RunHistoryPanel({ runs, currentRunId, busy, onSelect, onRetry, onCancel, onRefresh, }: {
    runs: RunSummary[];
    ...
    return (<section className="run-history-panel workflow-tab-history-panel">
      ...
      <div className="run-history-list">
        {runs.slice(0, 20).map((run) => (<div key={run.id} className={`run-history-row ${currentRunId === run.id ? 'active' : ''}`}>
