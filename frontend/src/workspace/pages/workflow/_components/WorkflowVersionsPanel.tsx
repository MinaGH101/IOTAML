...
export function WorkflowVersionsPanel({ versions, workflowId, selectedVersionId, previewActive, busy, onSelect, onRestore, onDelete, onRefresh, onReturnToCurrent, }: {
    ...
    return (<section className="workflow-versions-panel">
      ...
      {previewActive && (<div className="workflow-version-preview-banner">
          <div>
      ...
      <div className="workflow-version-list">
        {versions.map((version) => (<div key={version.id} className={`workflow-version-row ${selectedVersionId === version.id ? 'active' : ''}`}>
