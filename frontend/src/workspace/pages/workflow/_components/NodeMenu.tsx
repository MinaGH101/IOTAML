...
export function NodeMenu({ registry, paletteCollapsed, setPaletteCollapsed, floatingLeftStyle, onCreateCustomNode, onEditCustomNode }: NodeMenuProps) {
    return (<div className={`left-stack ${paletteCollapsed ? 'left-stack-collapsed' : ''}`} style={floatingLeftStyle}>
      <button className="workflow-float-toggle workflow-float-toggle-left" type="button" onClick={() => setPaletteCollapsed((value) => !value)} title={paletteCollapsed ? 'باز کردن منوی چپ' : 'بستن منوی چپ'} aria-label={paletteCollapsed ? 'باز کردن منوی چپ' : 'بستن منوی چپ'} style={{
      ...
      <NodePalette nodes={registry} collapsed={paletteCollapsed} onToggle={() => setPaletteCollapsed((value) => !value)} onCreateCustomNode={onCreateCustomNode} onEditCustomNode={onEditCustomNode}/>
    </div>);
}
