...
import type { ComponentDefinitionDraft, ComponentVersion, RegistryNode, UserProfile, WorkflowComponent, } from '../../../../../../shared/types';
...
export function componentVersionFromSnapshot(snapshot: Record<string, unknown>, ownerUsername: UserProfile['username']): ComponentVersion {
    return {
...
export function groupComponentGraph({ nodes, edges, boundary, draft, componentNode, }: {
    nodes: Node[];
    edges: Edge[];
    ...
    return {
        ...
        edges: [
            ...edges.filter((edge) => !boundary.selected.has(edge.source) && !boundary.selected.has(edge.target)),
            ...rewiredIncoming,
            ...rewiredOutgoing,
        ],
    };
}
export function expandComponentGraph({ nodes, edges, componentNode, nonce, }: {
    nodes: Node[];
    edges: Edge[];
    componentNode: Node;
    nonce: string;
    ...
    if (!snapshot)
        return { ok: false as const, reason: 'missing_snapshot' as const };
    const graph = (snapshot.graph || {}) as FlowGraph;
    const sourceNodes = Array.isArray(graph.nodes) ? graph.nodes : [];
    const sourceEdges = Array.isArray(graph.edges) ? graph.edges : [];
    if (sourceNodes.length === 0)
        return { ok: false as const, reason: 'empty_graph' as const };
    const idMap = new Map(sourceNodes.map((node) => [
        String(node.id),
...
export function upgradeComponentGraph({ sourceNodeId, parentNodes, parentEdges, component, version, registryNode, }: {
    sourceNodeId: string;
    parentNodes: Node[];
    parentEdges: Edge[];
    ...
    const incompatible = parentEdges.some((edge) => ((edge.target === sourceNodeId && edge.targetHandle && !inputIds.has(String(edge.targetHandle)))
        || (edge.source === sourceNodeId && edge.sourceHandle && !outputIds.has(String(edge.sourceHandle)))));
    if (incompatible)
        return { ok: false as const, reason: 'incompatible_ports' as const };
    const existing = parentNodes.find((node) => node.id === sourceNodeId);
    const defaults = Object.fromEntries((registryNode.settingsSchema || []).map((item) => [item.name, item.default]));
    ...
    return {
        ok: true as const,
        nodes: parentNodes.map((node) => (node.id === sourceNodeId ? { ...node, data: replacementData } : node)),
        edges: parentEdges,
    };
}
