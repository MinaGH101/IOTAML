...
export function useComponentEditorNavigation(options: UseComponentEditorOptions, state: ComponentEditorLocalState) {
    ...
    const enterNode = useCallback(async (node: Node) => {
        ...
        try {
            const component = items.find((item) => item.id === componentId) || await componentsApi.get(componentId, projectId);
            const version = versionId
                ? await componentsApi.getVersion(componentId, versionId, projectId).catch(() => componentVersionFromSnapshot(snapshot, user.username))
