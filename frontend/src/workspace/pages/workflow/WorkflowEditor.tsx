...
export function WorkflowEditor({
  ...
  } = useWorkflowShellState();

  const graph = useWorkflowGraph({ readOnly: readOnly });
  const {
    nodes,
    documentNodes,
    setNodes,
  ...
  const refreshRegistry = useCallback(async () => {
    setCatalog(await workspaceApi.nodeCatalog(projectId));
  }, []);
  ...
  const document = useWorkflowDocument({
    ...
    autosavePaused: !project.can_edit || Boolean(components.editor),
    versionPreview,
    setVersionPreview,
    setMessage,
  });
  ...
  const interactiveTableRuntime = useInteractiveTableController({
    scope: `workflow:${document.currentWorkflowId ?? initialWorkflowId ?? 'draft'}`,
    nodes: documentNodes,
    outputs: allRunOutputs,
  ...
  return (
    <InteractiveTableRuntimeContext.Provider value={interactiveTableRuntime}>
    <div className="app-shell workflow-shell-page" style={layout.appStyle}>
      <WorkflowHeader
        ...
        createComponentDisabled={readOnly || Boolean(components.editor) || selectedIds.length < 2}
        readOnly={readOnly}
        runDisabled={busy || document.versionBusy || readOnly || Boolean(components.editor) || nodes.length === 0}
        runSelectedNode={Boolean(selectedNode)}
        runBusy={busy}
        onRun={execution.runWorkflow}
        boardOpen={analysisBoardOpen}
        onToggleBoard={() => setAnalysisBoardOpen((value) => !value)}
        saveDisabled={document.versionBusy || readOnly || Boolean(components.editor)}
