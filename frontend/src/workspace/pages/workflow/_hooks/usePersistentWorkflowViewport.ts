...
type SetViewport = (viewport: WorkflowViewport, options?: {
    duration?: number;
}) => unknown;
export function usePersistentWorkflowViewport({ storageScope, setViewport, }: {
    storageScope: string;
    setViewport: SetViewport;
}) {
    const storageKey = `${storageScope}:workflow`;
    const viewportRef = useRef<WorkflowViewport>(loadCanvasViewport(storageKey, { x: 0, y: 0, zoom: 1 }));
