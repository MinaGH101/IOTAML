...
export function useBoardItems(o: AnalysisBoardsOptions, c: ReturnType<typeof useBoardCollection>, createReference: (output: Output, nodeId: string | null, key: string) => Pick<AnalysisBoardItem, 'outputRef'>) {
    const addOutputToBoard = useCallback((output: Output, visibleIndex: number, destination: string) => { if (o.readOnly)
        return; const nodeId = output.node_id ? String(output.node_id) : o.selectedNodeId; const nodeOutputs = o.outputs.filter((item) => String(item.node_id || '') === String(nodeId || '')); const found = nodeOutputs.findIndex((item) => item === output); const outputIndex = found >= 0 ? found : visibleIndex; const key = boardOutputKey(output); const label = o.nodes.find((node) => node.id === nodeId)?.data?.label; c.updateBoardItems(destination, (items) => { const offset = items.length % 5; return [...items, { id: `board-${crypto.randomUUID()}`, nodeId, outputKey: key, outputIndex, outputTitle: boardOutputTitle(output, outputIndex), outputKind: String(output.kind || 'json'), sourceLabel: label ? String(label) : undefined, x: 28 + offset * 34, y: 28 + offset * 34, w: 440, h: 330, runId: o.currentRunId, ...createReference(output, nodeId, key), createdAt: new Date().toISOString() }]; }); c.setActiveBoardId(destination); c.setTargetBoardId(destination); o.setBoardOpen(true); o.setMessage(`خروجی به ${c.boards.find((b) => b.id === destination)?.name || 'برد اصلی'} اضافه شد`); }, [c.boards, c.setActiveBoardId, c.setTargetBoardId, c.updateBoardItems, createReference, o.currentRunId, o.nodes, o.outputs, o.readOnly, o.selectedNodeId, o.setBoardOpen, o.setMessage]);
    ...
    const duplicateItem = useCallback((item: AnalysisBoardItem) => { if (!o.readOnly)
        c.updateBoardItems(c.activeBoardId, (items) => [...items, { ...item, id: `board-${crypto.randomUUID()}`, x: item.x + 26, y: item.y + 26, createdAt: new Date().toISOString() }]); }, [c.activeBoardId, c.updateBoardItems, o.readOnly]);
