...
import type { Dataset, PortDefinition, RegistryNode, Run } from '../../shared/types';
import { compatiblePorts, resolveRegistryId } from '../_model/catalog';
import { normalizeOutputs, type Output } from '../../features/results/components/ResultsPanel';
...
export type NodeModalProps = {
    ...
    onPinnedChange: (nodeId: string, pinned: {
        enabled?: boolean;
        sample?: string;
    }) => void;
...
export function NodeModal({ node, workflowNodes, edges, registry, aliases, portCompatibility, datasets, availableColumns, availableIdColumns = [], inheritedIdColumn = null, availableRows = [], run, busy, onRunNode, onCancelRun, onParamsChange, onRename, onPinnedChange, onAddOutputToBoard, onInputSourceHandleChange, onClose }: NodeModalProps) {
    ...
    const currentStatus = run?.node_statuses?.[node.id]?.status || null;
    const currentHasRun = currentOutputs.length > 0 || Boolean(currentStatus && !['queued', 'running'].includes(currentStatus));
    ...
    const inputGroups = incoming.map((edge) => {
        ...
        const sourceStatus = run?.node_statuses?.[edge.source]?.status || null;
        const sourceHasRun = sourceOutputs.length > 0 || Boolean(sourceStatus && !['queued', 'running'].includes(sourceStatus));
        return { edge, sourceNode, sourceDefinition, sourcePorts, targetPort, selectedHandle, sourceHasRun, visibleOutputs: selectedOutputs.length ? selectedOutputs : (annotated.length ? [] : sourceOutputs) };
    });
    const pinned = (node.data.pinned || {}) as {
        enabled?: boolean;
        sample?: string;
    };
    ...
    return (<div className="node-modal-backdrop workflow-shell-backdrop n8n-node-backdrop" onClick={onClose}>
      <div className={`node-modal workflow-shell-popup n8n-node-modal ${categoryClassName(String(node.data.category || 'Data Input'))}`} role="dialog" aria-modal="true" aria-label={`تنظیمات ${String(node.data.label || '')}`} onClick={(event) => event.stopPropagation()}>
        ...
        <div ref={gridRef} className="node-modal-grid n8n-node-grid n8n-node-grid-resizable" style={{ '--output-fr': `${columns.output}fr`, '--settings-fr': `${columns.settings}fr`, '--input-fr': `${columns.input}fr` } as CSSProperties}>
          ...
          <NodeInputsPanel groups={inputGroups} portCompatibility={portCompatibility} onInputSourceHandleChange={onInputSourceHandleChange} onAddOutputToBoard={onAddOutputToBoard}/>
        </div>
      </div>
    </div>);
}
