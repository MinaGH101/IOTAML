...
import type { ComponentBoundaryPort, ComponentDefinitionDraft } from '../../shared/types';
...
function PortRows({ label, ports, onChange }: {
    label: string;
    ports: ComponentBoundaryPort[];
    onChange: (ports: ComponentBoundaryPort[]) => void;
}) {
    return (<div className="component-port-editor">
      ...
      {ports.map((port, index) => (<div className="component-port-row" key={port.id}>
...
export function CreateComponentDialog({ open, busy, selectedCount, initialInputs, initialOutputs, onClose, onCreate }: CreateComponentDialogProps) {
    ...
    return (<AppDialog open={open} title="ساخت کامپوننت قابل استفاده مجدد" description={`${selectedCount} نود انتخاب‌شده به یک کامپوننت نسخه‌دار تبدیل می‌شوند.`} width={760} onClose={onClose} closeDisabled={busy} footer={<>
...
export function ComponentVersionDialog({ open, busy, currentVersion, onClose, onSave }: ComponentVersionDialogProps) {
    ...
    return (<AppDialog open={open} title="ذخیره نسخه جدید کامپوننت" description="نسخه‌های قبلی تغییر نمی‌کنند و جریان‌های موجود همچنان به نسخه قبلی متصل می‌مانند." onClose={onClose} closeDisabled={busy} footer={<>
      <button type="button" className="secondary-button" disabled={busy} onClick={onClose}>انصراف</button>
