...
export function CreateProjectPage({
  ...
  const ensureProject = async () => {
    ...
    const project = await projectsApi.create({ ...draft, name: draft.name.trim() });
  ...
  const saveProject = async () => {
    ...
    try {
      if (createdProject) {
        const saved = await projectsApi.update(createdProject.id, { ...draft, name: draft.name.trim() });
  ...
  const importWorkflow = async (file: File) => {
    ...
    try {
      ...
      const workflow = await workspaceApi.createWorkflow({ name: imported.name || workflowName.trim() || 'Imported Workflow', project_id: project.id, graph: graph as unknown as Record<string, unknown> });
      await refreshProjectAssets(project.id);
